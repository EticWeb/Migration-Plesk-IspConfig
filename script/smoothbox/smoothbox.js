...
function TB_show(caption, url, rel) {
	...
	//alert(baseURL);
	//alert(url.match(/(.+)?/)?'yes':'no');
	// regex to check if a href refers to an image
	var imageURL = /\.(jpe?g|png|gif|bmp)/gi;

	// check for images
	/* Modif EticWeb
	 * Correction bug sur les request contenant un fichier dans les paramettres ex:photo=titi.jpg=vignette=1
	 */
	if (baseURL != 1 && baseURL.match(imageURL) ) {
		//alert('yes'+baseURL);
